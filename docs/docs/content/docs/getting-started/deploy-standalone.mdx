...
- In standalone Traefik mode, `/api/v1/traefik-config` should be served from Middleware Manager's Traefik/standalone path and must not require Pangolin.
- `PANGOLIN_API_URL` may still exist in the container environment, but it should not be consulted while `active_data_source` is `traefik`.
